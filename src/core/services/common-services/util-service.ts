...
export const utilService = {
  ...
  uniqueArray(array: any[], uniqueField: string = "id") {
    return array.reduce((acc, current) => {
      const x = acc.find(
        (item: any) => item[uniqueField] === current[uniqueField]
      );
      if (!x) {
  ...
  downloadURI(uri: string, name: string = "fileDownloaded") {
  ...
  countValuedField(obj: { [key: string]: any }): number {
    let count = 0;
    obj &&
      Object.keys(obj).forEach((key: string) => {
        if (!key.includes("Value") && !this.isEmpty(obj[key])) {
          count = count + 1;
        }
      });
    return count;
